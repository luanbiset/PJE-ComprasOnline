...
SET @sql := (
    SELECT IF(
        ...
        'CREATE TABLE TAB_EMAIL_CLIENTE (
			...
			CONSTRAINT EMACLI_CK01 CHECK (FLG_ATIVO IN (0,1))
...
SET @sql2 := (
    SELECT IF(
        ...
        'CREATE INDEX EMACLI_IDX01 ON TAB_EMAIL_CLIENTE(IDT_CLIENTE,FLG_ATIVO)'
    )
);
...
INSERT INTO pje_adm.TAB_EMAIL_CLIENTE (IDT_CLIENTE,DES_EMAIL,FLG_ATIVO,DAT_CRIACAO,DAT_ATUALIZACAO,IDT_USR_CRIACAO,IDT_USR_ALTERACAO) VALUES (@IDT_CLIENTE,'teste@gmail.com',1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1,NULL)
ON DUPLICATE KEY UPDATE FLG_ATIVO = VALUES(FLG_ATIVO), DAT_ATUALIZACAO = CURRENT_TIMESTAMP;
COMMIT;
