...
SET @sql := (
    SELECT IF(
        ...
        'CREATE TABLE TAB_CIDADE (
            IDT_CIDADE INT NOT NULL AUTO_INCREMENT 					 COMMENT ''[NOT_SECURITY_APPLY] - Chave primária da tabela Cidade.'',
			...
			CONSTRAINT CID_CK01 CHECK (FLG_ATIVO IN (0,1))
...
SET @sql2 := (
    SELECT IF(
        ...
        'CREATE INDEX CID_IDX01 ON TAB_CIDADE(IDT_UF,FLG_ATIVO)'
    )
);
...
VALUES
(@IDT_UF, 2900108, 'Abaíra', 1, 1),
(@IDT_UF, 2900207, 'Abaré', 1, 1),
(@IDT_UF, 2900306, 'Acajutiba', 1, 1),
(@IDT_UF, 2900405, 'Água Fria', 1, 1),
(@IDT_UF, 2900702, 'Alagoinhas', 1, 1),
(@IDT_UF, 2901007, 'Amargosa', 1, 1),
(@IDT_UF, 2903201, 'Barreiras', 1, 1),
(@IDT_UF, 2905701, 'Camaçari', 1, 1),
(@IDT_UF, 2910800, 'Feira de Santana', 1, 1),
(@IDT_UF, 2913606, 'Ilhéus', 1, 1),
(@IDT_UF, 2914802, 'Itabuna', 1, 1),
(@IDT_UF, 2927408, 'Salvador', 1, 1),
(@IDT_UF, 2933307, 'Vitória da Conquista', 1, 1) ON DUPLICATE KEY UPDATE COD_CIDADE = VALUES(COD_CIDADE), NAM_CIDADE = VALUES(NAM_CIDADE);
commit;